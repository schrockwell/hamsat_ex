...
L.GreatCircle = L.Circle.extend({
  ...
  bindTo: function (object, event = "drag") {
    ...
    object.on(
      ...
      function (ev) {
        ...
        var noredraw =
          ll.lat == this._position.lat && ll.lng == this._position.lng; //keeps this from redrawing when nothing has really changed -- otherwise redraw will fire twice on addMap and bindTo
  ...
  _destination_from_bearing: function (
    lat,
    lng,
    bearing,
    distance,
    ...
    if (typeof round_off != "undefined") {
      return [
        this._round(rad2deg * lat2, round_off),
        this._round(rad2deg * lng2, round_off),
      ];
    } else {
  ...
  redraw: function () {
    ...
    var l1 = this._destination_from_bearing(
      lat,
      lng,
      0,
      this._options.radius * this._m2km,
    ...
    var l2 = this._destination_from_bearing(
      lat,
      lng,
      360,
      this._options.radius * this._m2km,
    ...
    var l3 = this._destination_from_bearing(
      lat,
      lng,
      180,
      this._options.radius * this._m2km,
    ...
    var l4 = this._destination_from_bearing(
      lat,
      lng,
      -180,
      this._options.radius * this._m2km,
    ...
    if (this._options.maxCopies > -1)
      this._copies =
        this._copies > this._options.maxCopies
          ? this._options.maxCopies
          : this._copies;
      ...
      for (var copy = this._copies * -1; copy <= this._copies; copy++) {
        ...
        if (this._clipStatus == 4) {
          ...
          if (copy == this._copies * -1) {
            ...
            this._latLngsM[this._latLngsM.length - 1] = this._latLngsM[
              this._latLngsM.length - 1
            ].concat(this._latLngs);
          }
          this._latLngs = [];
      ...
      for (var copy = this._copies * -1; copy <= this._copies; copy++) {
        if (copy == 0) {
          ...
          if (typeof this._circles[copy] == "undefined") {
            this._circles[copy] = new L.circle(
              [lat, lng + 360 * copy],
              this._options
            );
            if (this._addedToMap) {
