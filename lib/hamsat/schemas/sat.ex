defmodule Hamsat.Schemas.Sat do
  ...
  schema "satellites" do
    ...
    field :slug, :string
    field :modes, {:array, Ecto.Enum}, values: Hamsat.Modulation.sat_values()
  ...
  def upsert_changeset(sat \\ %__MODULE__{}, attrs) do
    sat
    |> cast(attrs, [:name, :number, :slug, :nasa_name, :modes])
    |> put_nasa_name()
    |> validate_required([:name, :number, :slug, :nasa_name, :modes])
